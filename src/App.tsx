...
const NAV_ITEMS = [
  { to: '/', label: '채팅' },
  { to: '/admin/pending-recipes', label: '제출 검수' },
  { to: '/admin/recipes', label: '운영 레시피' },
];

export default function App() {
  return (
    <div className="flex h-screen bg-(--color-lighter)">
      <aside className="flex w-52 flex-col bg-(--color-main) text-white">
        <div className="p-5">
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-hidden p-4 md:p-6">
        <div className="flex h-full flex-col overflow-hidden rounded-lg bg-(--color-main-bg) shadow-xl">
          <Outlet />
        </div>
      </main>
    </div>
